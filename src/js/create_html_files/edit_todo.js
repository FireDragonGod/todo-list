...
import { updateTodoValues, todoValue } from '../create_todo/update_todo';
import { matchValue, todoLists } from '../create_todo/todo_create';
...
class EditTodo {
  ...
  editTodoForm = function createAnElementThatPopUpWhenClickingEditImage(
    ...
    titleInput.setAttribute('id', 'task_name');
    titleInput.setAttribute('placeholder', 'todo-list');
    ...
    descriptionInput.setAttribute('name', 'task_description');
    descriptionInput.setAttribute('minlength', '');
    descriptionInput.setAttribute('maxlength', '248');
    descriptionInput.setAttribute('id', 'task_description');
    descriptionInput.setAttribute('placeholder', 'Provide more info about the task')
    ...
    lowInput.setAttribute('id', 'low_priority');
    lowInput.setAttribute('name', 'priority_level');
    lowInput.setAttribute('value', 'low_priority');

    labelMedium.textContent = 'Medium Priority';
    labelMedium.setAttribute('for', 'medium_priority');
    mediumInput.setAttribute('type', 'radio');
    mediumInput.setAttribute('id', 'medium_priority');
    mediumInput.setAttribute('name', 'priority_level');
    mediumInput.setAttribute('value', 'medium_priority');

    labelHigh.textContent = 'High Priority';
    labelHigh.setAttribute('for', 'high_priority');
    highInput.setAttribute('type', 'radio');
    highInput.setAttribute('id', 'high_priority');
    highInput.setAttribute('name', 'priority_level');
    highInput.setAttribute('value', 'high_priority');
    ...
    dueDateInput.setAttribute('min', ``);
    dueDateInput.setAttribute('max', '2100-12-31');
    dueDateInput.setAttribute('value', `${dueDate}`);

    labelSubmit.setAttribute('for', 'submit_todo');
    submitInput.textContent = 'Change'
    submitInput.setAttribute('id', 'submit_todo');
    submitInput.setAttribute('name', 'submit_todo');
    submitInput.setAttribute('type', 'submit');

    lowInput.checked = matchValue.match('low_priority', priority);
    mediumInput.checked = matchValue.match('medium_priority', priority);
    highInput.checked = matchValue.match('high_priority', priority);
    ...
    submitInput.addEventListener(
      ...
      (event) => {
        ...
        updateTodoValues.conditionValues(
          event,
          titleInputValue,
          dataKey,
          priorityLevelStore,
    ...
    appendMultipleChildren(liPriority, [priorityInput]);
    appendMultipleChildren(liLow, [lowInput, labelLow]);
    appendMultipleChildren(liMedium, [mediumInput, labelMedium, ])
    appendMultipleChildren(liHigh, [highInput, labelHigh, ])
